...
const NewsCard = ({
  ...
  return (
    <motion.article
      ...
      className="group w-full rounded-xl overflow-hidden bg-[#091413] border border-[#182B28] shadow-md hover:border-[#A4B885]/60 hover:shadow-lg transition-all duration-300 flex flex-col"
    >
      {/* Top: Shorter image preview cover */}
      <div
        id={`news-image-container-${item.id}`}
        className="relative w-full h-44 sm:h-52 bg-[#050C0B] overflow-hidden cursor-pointer"
        onClick={handleRead}
      >
        {item.image ? (
          <img
      ...
      <div id={`news-body-${item.id}`} className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
        ...
        <footer
          ...
          className="mt-6 pt-4 border-t border-[#182B28] flex items-center gap-3"
        >
